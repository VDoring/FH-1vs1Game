...
int main()
{
	...
	if (First_select == 1) { //1번. 게임시작
		...
		while (1) { //반복문 2 시작
			...
			if (Attack_Number == 3) { //차기 <추후 찬 부분 충격으로 인해 지속적인 데미지를 넣게끔 추가하기>
				Sleep(700);
				printf("\n\n==================================[턴 수 : %d]=================================\n\n", Turn);
				Sleep(1000);
				puts("당신은 상대를 찼습니다.");
			...
			if (Attack_Number == 4) { //베기
				...
				if (Slash_Attack >= 3 && Slash_Attack <= 6) { //3~6 공격성공
					printf("베었습니다.");
					Damage = Player_ATK / 2;
					Damage -= Monster1_DEF / 3;
					Sleep(1000);
				...
				if (Slash_Attack >= 7 && Slash_Attack <= 9) { //7~9 강한공격성공
					printf("제대로 베었습니다.");
					Damage = Player_ATK;
					Damage -= Monster1_DEF / 3;
					Sleep(1000);
			...
			if (Turn % 2 == 0) { //(턴이 짝수일때)몬스터 공격 턴
				...
				if (Monster_Attack >= 8 && Monster_Attack <= 9) {
					...
					printf("\n몬스터가 강력한 공격을 했습니다!");
					Damage = Monster1_ATK;
					Damage -= (Player_DEF / 2) + Damage / 4; // Player_DEF / 2.5와 같음.  여기엔 Release버전으로 맞춤. 데미지 적음
					Player_HP -= Damage;
					Sleep(1000);
					printf("\n\n당신은 %d만큼의 데미지를 입었습니다.", Damage);
					Sleep(1500);
