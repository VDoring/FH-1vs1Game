...
void title() { //메인타이틀(첫화면) 표시
	int x = 44, y = 5;
	system("mode con cols=120 lines=40"); //**창 가로크기 120, 세로크기 40 설정
	gotoxy(x, y + 0); printf("______               ___  ___"); Sleep(100); //총 가로길이 29
	...
	gotoxy(x, y + 8); printf("        [1] 게임 시작"); Sleep(7);
	gotoxy(x, y + 10); printf("        [2] 초기화"); Sleep(7);
	gotoxy(x, y + 12); printf("        [3] 종료"); Sleep(7);
	gotoxy(x, y + 15); printf("  Input  :  "); Sleep(120);
	gotoxy(x + 13, y + 15); scanf("%d", &First_select);  //번호 입력
	if (First_select != 1 && First_select != 2 && First_select != 3) {
		gotoxy(x + 4, y + 18); printf("[다시 입력하십시오]"); Sleep(700);
	}
}

void choose_character() { //캐릭터 선택
	system("mode con cols=110 lines=43"); //**창 가로크기 120, 세로크기 40 설정
	printf("\n[캐릭터를 선택하십시오]------------------\n"); //캐릭터 선택 문구
	Sleep(100);
