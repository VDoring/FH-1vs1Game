...
void player_act() { //플레이어의 턴. 행동을 취한다
	while (1) {
		...
		gotoxy(x + 4, y + 2); printf("== 무엇을 할까? ==");
		Sleep(200);
		gotoxy(x - 20, y + 6); printf("[1] 몸통박치기        [2] 연속할퀴기        [3] 어퍼킥        [4] 참격"); //어떻게 공격할건지 선택
...
void player_AttackNum_1() { // 1. 때리기 선택시..
	...
	Bodyhit_Attack = (int)Bodyhit_Attack_random_number % 8; //랜덤숫자 0~7
}
...
void player_AttackNum_2() { // 2. 할퀴기 선택시..
	...
	printf("\n\n==================================[턴 수 : %d]=================================\n\n", Turn);
	Sleep(1000);
	printf("당신은 상대를 %d번 할퀴었습니다.", Scratch_counts); //여기 몇번 할켰는지 나오는 변수가 이상하게 작동함!!===============================================
