		...
		if (First_select == 1) { //1번. 게임시작 선택시..
			...
			while (1) {
				...
				if (Attack_Number == 2) { // 2. 할퀴기 선택시..
					player_Attacknum_2_counts();
					int Scratch_counts;
					if (Scratch_Attack_counts == 0) { //만약 랜덤숫자가 0일때
						Scratch_counts = 1;
						player_AttackNum_2(); //공격을 1번만 함(랜덤숫자와 공격횟수가 다른 이유는 랜덤함수가 나타낼 수 있는 가장 낮은 숫자가 0이기 때문. 1부터 나타내는건 불가하므로 if문을 이용하여 따로 사용한다)
					}
					if (Scratch_Attack_counts == 1) {
						Scratch_counts = 2;
						player_AttackNum_2(); //공격을 2번만 함
					}
					if (Scratch_Attack_counts == 2) {
						Scratch_counts = 3;
						player_AttackNum_2(); //공격을 3번 함
					}
					if (Scratch_Attack_counts == 3) {
						Scratch_counts = 4;
						player_AttackNum_2(); //공격을 4번 함
					}
					if (Scratch_Attack_counts == 4) {
						Scratch_counts = 5;
						player_AttackNum_2(); //공격을 5번 함
					}
					endline_player(); //몬스터에게 입힌 데미지, 남은 HP 출력
				}
