...
void player_Attacker_visual() {
	...
	gotoxy(x, y+39); printf("어택커 아스키아트");
}

void player_Defender_visual() {
	int x = 0, y = 0; system("cls");
	gotoxy(x, y + 39); printf("디펜더 아스키아트");
}

void player_Allrounder_visual() {
	int x = 0, y = 0; system("cls");
	gotoxy(x, y + 39); printf("올라운더 아스키아트");
}
...
void player_act() { //플레이어의 턴. 행동을 취한다
	while (1) {
		...
		int x = 47, y = 0;
		gotoxy(x - 43, y+40); printf("=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"); Sleep(100);
		gotoxy(x + 30, y + 42); printf("== 무엇을 할까? =="); Sleep(200);
		gotoxy(x + 3, y + 46); printf("[1] 몸통박치기        [2] 연속할퀴기        [3] 어퍼킥        [4] 참격"); //어떻게 공격할건지 선택
		gotoxy(x + 38, y + 49); printf(": ");
		gotoxy(x + 39, y + 49); scanf("%d", &Attack_Number);
