...
void player_monster_visual(); //플레이어가 행동을 선택하기 전에 뜨는 아스키아트식 몬스터의 모습을 출력(채력도 함께 출력된다)
